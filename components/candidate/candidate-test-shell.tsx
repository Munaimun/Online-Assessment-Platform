...
interface CandidateTestShellProps {
  children: React.ReactNode;
  centerTitle?: string;
}

export function CandidateTestShell({ children, centerTitle = "Akij Resource" }: CandidateTestShellProps) {
  return (
    <div className="min-h-screen bg-[#f4f5f7] text-[#354052]">
      <header className="border-b border-[#eceef2] bg-white">
        <div className="mx-auto flex h-18 w-full max-w-310 items-center justify-between px-4 md:px-6">
          <Link href="/candidate/dashboard" className="leading-none">
          ...
          <p className="text-[18px] font-semibold text-[#364253] md:text-[44px]">{centerTitle}</p>

          <ProfileMenu name="Jhon Smith Doe..." refId="12341341" compact />
        </div>
      </header>

      <div className="mx-auto w-full max-w-310 px-4 py-8 md:px-6">{children}</div>
